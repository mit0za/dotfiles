...
--   awful.spawn.easy_async_with_shell("amixer -D pipewire sget Master | grep 'Right:' | awk -F'[][]' '{ print $2 }'",
--     function(stdout)
...
volume_slider:connect_signal("property::value", function(_, vol)
  awful.spawn("amixer -D pipewire sset Master " .. vol .. "%", false)
  -- Update textbox widget text
  volume_osd_value.text = vol .. "%"
  awesome.emit_signal("module::volume_osd_value", vol)
end)
...
local update_mic = function()
  awful.spawn.easy_async_with_shell("amixer -D pipewire sget Capture | grep 'Right:' | awk -F'[][]' '{ print $2 }'",
    function(stdout)
      local volume = tonumber(stdout:match("%d+"))
...
mic_slider:connect_signal("property::value", function(_, mic_vol)
  awful.spawn("amixer -D pipewire sset Capture " .. mic_vol .. "%", false)
  -- Update textbox widget text
